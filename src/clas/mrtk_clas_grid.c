...
static double Interp1(const double* x, const double* y, int nx, int ny, double xi) {
    int i;
    double dx;
...
static int get_mops(const double lat, const double doy, double* pre, double* temp, double* wpre, double* ltemp,
                    double* wvap) {
    int i;
...
static double get_Twv(const double Ts, const double es) { return 2.2768 * (1255.0 / Ts + 0.05) * es * 0.001; }
...
int get_stTv(gtime_t t, const double lat, const double hs, const double hg, double* tdv, double* twv) {
...
static int pickup_candidates_of_grid(const clas_ctx_t* ctx, grid_info* gridinfo, const int network, const double* pos,
                                     int ch) {
    ...
    for (inet = start; inet < end; ++inet) {
        for (i = 0; i < CLAS_MAX_GP; ++i) {
            ...
            if (fabs(-1.0 - ctx->grid_pos[inet][i][0]) < 0.0001 && fabs(-1.0 - ctx->grid_pos[inet][i][1]) < 0.0001 &&
                fabs(-1.0 - ctx->grid_pos[inet][i][2]) < 0.0001) {
                break;
            }
            /* skip grids with non-zero height or invalid status */
            if (fabs(ctx->grid_pos[inet][i][2]) > 0.0001 || ctx->grid_stat[ch][inet][i] == 0) {
                continue;
            }

            /* distance to grid (m) */
            dd[0] = RE_WGS84 * (pos[0] - ctx->grid_pos[inet][i][0] * D2R);
            dd[1] = RE_WGS84 * (pos[1] - ctx->grid_pos[inet][i][1] * D2R) * cos(pos[0]);
            if ((d = MAX(norm(dd, 2), 1.0)) > MAX_DIST * 1000.0) {
                continue;
            }

            trace(NULL, 5,
                  "get_grid_index: inet=%2d, pos=%3.3f %3.3f, "
                  "dist=%6.3f[km]\n",
                  inet, ctx->grid_pos[inet][i][0], ctx->grid_pos[inet][i][1], d / 1000.0);
...
static int find_grid_surround_pos(grid_info** gridindex, int n, grid_info* gridinfo) {
    ...
    first_index = (gridinfo[1].network == gridinfo[2].network && gridinfo[0].network != gridinfo[1].network) ? 1 : 0;

    /* find 2nd grid in same network */
    for (i = 1, gridindex[0] = &gridinfo[first_index], flag = 0; i < n; ++i) {
...
static int is_exists_inside(const double* pos, int n, grid_info** gridindex, grid_info* nearestgrid) {
    int flag = 0xff; /* always true — upstream uses this as disabled stub */

    if (flag == 0x00) {
        if (nearestgrid->weight < gridindex[0]->weight) {
            trace(NULL, 2,
                  "change to the nearest grid: inet=%d, index=%d, "
...
static void output_selected_grid(const double* pos, int n, grid_info** gridindex, double* weight) {
    ...
    for (i = 0; i < n; i++) {
        trace(NULL, 1,
              "selected_grid%d: inet=%d, weight=%.2f, "
 ...
 *---------------------------------------------------------------------------*/
extern int clas_get_grid_index(clas_ctx_t* ctx, const double* pos, clas_grid_t* grid, int gridsel, gtime_t obstime) {
    ...
    for (ch = 0; ch < CLAS_CH_NUM; ch++) {
        nch[ch] = pickup_candidates_of_grid(ctx, gridinfo[ch], grid->network, pos, ch);
    }
    ch = nch[0] >= nch[1] ? 0 : 1;
    n = nch[ch];
...
static int stec_data(stec_t* stec, gtime_t time, int sat, double* iono, double* rate, double* rms, double* quality,
                     int* slip) {
    int k, flag;
    double tt;
 ...
 *  return: 1=ok, 0=error
 *---------------------------------------------------------------------------*/
extern int clas_stec_grid_data(clas_corr_t* corr, const int* index, gtime_t time, int sat, int n, const double* weight,
                               const double* Gmat, const double* Emat, double* iono, double* rate, double* var,
                               int* brk) {
    int i, slip;
    ...
    if (n == 1) {
        if (!stec_data(&corr->stec[index[0]], time, sat, iono, rate, rms, quals, &slip)) {
            free(ionos);
            free(rates);
            free(rms);
            free(quals);
    ...
    for (i = 0; i < n; i++) {
        if (!stec_data(&corr->stec[index[i]], time, sat, ionos + i, rates + i, rms + i, quals + i, &slip)) {
            free(ionos);
            free(rates);
            free(rms);
            free(quals);
...
static int trop_data(zwd_t* z, gtime_t time, double* ztd, double* zwd, double* quality, int idx, int* valid) {
    double tt;
    int k = 0;
    ...
    if (z->data[k].ztd == CSSR_INVALID_VALUE || z->data[k].zwd == CSSR_INVALID_VALUE) {
        trace(NULL, 2, "invalid trop correction %s ztd=%.2f zwd=%.2f\n", time_str(time, 0), z->data[k].ztd,
              z->data[k].zwd);
        return 0;
    }
 ...
 *  return: 1=ok, 0=error
 *---------------------------------------------------------------------------*/
extern int clas_trop_grid_data(clas_corr_t* corr, const int* index, gtime_t time, int n, const double* weight,
    ...
    if (n == 1) {
        ret = trop_data(&corr->zwd[index[0]], time, ztd, zwd, quals, index[0], valid);
        free(zwds);
        free(ztds);
        free(quals);
        free(valid);
    ...
    for (i = 0; i < n; i++) {
        if (!trop_data(&corr->zwd[index[i]], time, ztds + i, zwds + i, quals + i, index[i], valid + i)) {
            free(zwds);
            free(ztds);
            free(quals);
            free(valid);
...
extern int readblqgrid(const char* file, clas_ctx_t* ctx) {
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        ...
        if (net < 1 || net > CLAS_MAX_NETWORK || gridno < 1 || gridno > CLAS_MAX_GP) {
