...
/* extend sign ---------------------------------------------------------------*/
static int32_t exsign(uint32_t v, int bits) { return (int32_t)(v & (1 << (bits - 1)) ? v | (~0u << bits) : v); }
/* checksum ------------------------------------------------------------------*/
 ...
 * notes  : refer [1][3]
 *-----------------------------------------------------------------------------*/
static int decode_track_stat(uint32_t stat, int* sys, int* code, int* track, int* plock, int* clock, int* parity,
                             int* halfc) {
...
static int decode_rawsbasframeb(raw_t* raw) { return decode_rawwaasframeb(raw); }
/* decode GPSEPHEMB ----------------------------------------------------------*/
...
static int decode_galinavephemerisb(raw_t* raw) {
    ...
    eph.A = SQR(sqrtA);
    eph.svh = ((svh_e5b << 4) | (dvs_e5b << 3) | (svh_e1b << 1) |
               dvs_e1b); /* bit0:E1-B DVS bit1-2:E1-B SVH bit3:E5b DVS bit4-5:E5b SVH*/
...
static int decode_rgeb(raw_t* raw) {
    ...
    for (i = 0, p += 20; i < nobs; i++, p += 44) {
        ...
        if ((index = obsindex(&raw->obs, raw->time, sat)) >= 0) {
            ...
            raw->obs.data[index].SNR[freq] = 0.0 <= snr && snr < 255.0 ? (uint16_t)(snr / SNR_UNIT + 0.5) : 0;
...
static int decode_oem4(raw_t* raw) {
    ...
    switch (type) {
        ...
        case ID_QZSSEPHEMERIS:
            return decode_qzssephemerisb(raw);
        case ID_QZSSIONUTC:
