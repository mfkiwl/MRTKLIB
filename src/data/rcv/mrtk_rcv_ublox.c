...
static int decode_rxmrawx(raw_t* raw) {
    ...
    if (raw->outtype) {
        ...
        sprintf(raw->msgtype, "UBX RXM-RAWX  (%4d): time=%s nmeas=%d ver=%d", raw->len, tstr, nmeas, ver);
    }
    /* time tag adjustment option (-TADJ) */
    if ((q = strstr(raw->opt, "-TADJ="))) {
        sscanf(q, "-TADJ=%lf", &tadj);
    ...
    for (i = 0, p += 16; i < nmeas && n < MAXOBS; i++, p += 32) {
        ...
        slip = lockt == 0 || lockt * 1E-3 < raw->lockt[sat - 1][idx] || halfc != raw->halfc[sat - 1][idx] ||
               (std_slip && cpstd >= std_slip);
...
static int decode_rxmsfrbx(raw_t* raw) {
    ...
    if (raw->outtype) {
        sprintf(raw->msgtype, "UBX RXM-SFRBX (%4d): sys=%d prn=%3d", raw->len, U1(p), U1(p + 1));
    }
    if (!(sys = ubx_sys(U1(p)))) {
...
static int decode_trksfrbx(raw_t* raw) {
    ...
    if (raw->outtype) {
        sprintf(raw->msgtype, "UBX TRK-SFRBX (%4d): sys=%d prn=%3d", raw->len, U1(p + 1), U1(p + 2));
    }
...
static int decode_rxmqzssl6(raw_t* raw, rtcm_t* rtcm) {
    ...
    if (raw->outtype) {
        sprintf(raw->msgtype, "UBX RXM-QZSSL6(%4d): ver=%d prn=%2d msg=%s cn0=%.1f nerr=%2d stat=%d", raw->len, ver,
                prn, msg ? "L6E" : "L6D", cn0, nerr, stat);
    }
    if (strstr(raw->opt, "-OUT_QZSSL6")) {
        if (stat >= 1) {
        ...
        if (stat == 1) {
            trace(NULL, 2, "$CH,%.3f,%s,%d,%d,%.1f,%.9f,%.3f,%.3f,%d,%d\n", time, msg ? "L6E" : "L6D", prn, 0, cn0, 0.0,
                  0.0, 0.0, count[prn - 192][0], count[prn - 192][1]);
            for (i = 0; i < 250; i++) {
...
static int decode_ubx(raw_t* raw, rtcm_t* rtcm) {
    ...
    switch (type) {
        ...
        case ID_RXMQZSSL6:
            return decode_rxmqzssl6(raw, rtcm);
        case ID_NAVSOL:
    ...
    if (raw->outtype) {
        sprintf(raw->msgtype, "UBX 0x%02X 0x%02X (%4d)", type >> 8, type & 0xFF, raw->len);
    }
    return 0;
}
/* sync code -----------------------------------------------------------------*/
...
extern int gen_ubx(const char* msg, uint8_t* buff) {
    const char* cmd[] = {"PRT",   "USB",    "MSG",    "NMEA",   "RATE",   "CFG",  "TP",        "NAV2",
                         "DAT",   "INF",    "RST",    "RXM",    "ANT",    "FXN",  "SBAS",      "LIC",
                         "TM",    "TM2",    "TMODE",  "EKF",    "GNSS",   "ITFM", "LOGFILTER", "NAV5",
                         "NAVX5", "ODO",    "PM2",    "PWR",    "RINV",   "SMGR", "TMODE2",    "TMODE3",
                         "TPS",   "TXSLOT", "VALDEL", "VALGET", "VALSET", ""};
    const uint8_t id[] = {0x00, 0x1B, 0x01, 0x17, 0x08, 0x09, 0x07, 0x1A, 0x06, 0x02, 0x04, 0x11, 0x13,
                          0x0E, 0x16, 0x80, 0x10, 0x19, 0x1D, 0x12, 0x3E, 0x39, 0x47, 0x24, 0x23, 0x1E,
                          0x3B, 0x57, 0x34, 0x62, 0x3D, 0x71, 0x31, 0x53, 0x8c, 0x8b, 0x8a};
    const int prm[][32] = {
        ...
        {FU1, FU1, FU2, FI4, FI4, FI4, FI1, FI1, FI1, FU1, FU4,
         FU4, FU4, FU1, FU1, FU1, FU1, FU1, FU1, FU1, FU1},           /* TMODE3 */
