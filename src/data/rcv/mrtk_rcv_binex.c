...
static int32_t I4(uint8_t* p) { return (int32_t)U4(p); }
...
static int decode_bnx_00(raw_t* raw, uint8_t* buff, int len) {
    ...
    while (p - buff < len) {
        ...
        if (fid <= 0x0c || (fid >= 0x0f && fid <= 0x1c) || (fid >= 0x20 && fid <= 0x22) || fid == 0x7f) {
...
static int decode_bnx_01(raw_t* raw, uint8_t* buff, int len) {
    ...
    if (raw->outtype) {
        ...
        sprintf(raw->msgtype + strlen(raw->msgtype), " subrec=%02X prn=%d", srec, prn);
    }
    switch (srec) {
        case 0x00:
...
static uint8_t* decode_bnx_7f_05_obs(raw_t* raw, uint8_t* buff, int sat, int nobs, obsd_t* data) {
    ...
    switch (sys) {
        ...
        default:
            codes = NULL;
            break; /* skip unsupported sat data */
    }
    for (i = 0; i < nobs; i++) {
        flag = getbitu(p, 0, 1);
...
static int sync_bnx(uint8_t* buff, uint8_t data) {
    ...
    return buff[0] == BNXSYNC2 && (buff[1] == 0x00 || buff[1] == 0x01 || buff[1] == 0x02 || buff[1] == 0x03 ||
...
static int readbnxfp(FILE* fp, gtime_t ts, gtime_t te, double tint, const char* opt, int flag, int index, char* type,
...
/* read BINEX file -----------------------------------------------------------*/
static int readbnxfile(const char* file, gtime_t ts, gtime_t te, double tint, const char* opt, int flag, int index,
                       char* type, obs_t* obs, nav_t* nav, sta_t* sta, const prcopt_t* popt) {
    FILE* fp;
    int cstat, stat;
    char tmpfile[1024];
 ...
 *-----------------------------------------------------------------------------*/
extern int readbnxt(const char* file, int rcv, gtime_t ts, gtime_t te, double tint, const char* opt, obs_t* obs,
                    nav_t* nav, sta_t* sta, const prcopt_t* popt) {
    int i, n, stat = 0;
    const char* p;
