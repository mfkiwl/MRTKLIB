...
static const int16_t x1[] = {-75, -65, -55, -50, -45, -40, -35, -30, -25, -20, -15, -10, -5, 0,
                     ...
                     x5[] = {-180, -175, -170, -165, -160, -155, -150, -145, -140, -135, -130, -125, -120, -115, -110,
...
static int decode_sbstype26(const sbsmsg_t* msg, sbsion_t* sbsion) {
    ...
    if (band > MAXBAND || sbsion[band].iodi != (int)getbitu(msg->msg, 217, 2)) {
        return 0;
    }
...
static void readmsgs(const char* file, int sel, gtime_t ts, gtime_t te, sbs_t* sbs) {
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        if (sscanf(buff, "%d %lf %d", &week, &tow, &prn) == 3 && (p = strstr(buff, ": "))) {
            p += 2; /* rtklib form */
        } else if (sscanf(buff, "%d %lf %lf %lf %lf %lf %lf %d", &prn, ep, ep + 1, ep + 2, ep + 3, ep + 4, ep + 5,
                          &msg) == 8) {
        ...
        for (i = 0; *(p - 1) && *p && i < 29; p += 2, i++) {
            if (sscanf(p, "%2X", &b) == 1) {
                sbs->msgs[sbs->n].msg[i] = (uint8_t)b;
            }
        }
...
/* search igps ---------------------------------------------------------------*/
static void searchigp(gtime_t time, const double* pos, const sbsion_t* ion, const sbsigp_t** igp, double* x,
                      double* y) {
    int i, latp[2], lonp[4];
    ...
    if (-55.0 <= lat && lat < 55.0) {
        ...
        *y = (lat - latp[0]) / 10.0;
        if (75.0 <= lat && lat < 85.0) {
            ...
            for (i = 0; i < 4; i++) {
                lonp[i] = (int)floor(lon / 90.0) * 90;
            }
        } else if (lat < -85.0) {
            for (i = 0; i < 4; i++) {
                lonp[i] = (int)floor((lon - 50.0) / 90.0) * 90 + 40;
            }
        }
    }
    for (i = 0; i < 4; i++) {
...
double sbstropcorr(gtime_t time, const double* pos, const double* azel, double* var) {
    ...
    if (zh == 0.0 || fabs(pos[0] - pos_[0]) > 1E-7 || fabs(pos[1] - pos_[1]) > 1E-7 || fabs(pos[2] - pos_[2]) > 1.0) {
...
static int sbslongcorr(gtime_t time, int sat, const sbssat_t* sbssat, double* drs, double* ddts) {
    const sbssatp_t* p;
    double t;
    int i;
...
/* fast correction -----------------------------------------------------------*/
static int sbsfastcorr(gtime_t time, int sat, const sbssat_t* sbssat, double* prc, double* var) {
    const sbssatp_t* p;
    double t;
