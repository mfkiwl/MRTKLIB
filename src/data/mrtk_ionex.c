...
static tec_t* addtec(const double* lats, const double* lons, const double* hgts, double rb, nav_t* nav) {
    ...
    if (nav->nt >= nav->ntmax) {
        ...
        if (!(nav_tec = (tec_t*)realloc(nav->tec, sizeof(tec_t) * nav->ntmax))) {
            trace(NULL, 1, "readionex malloc error ntmax=%d\n", nav->ntmax);
            free(nav->tec);
            nav->tec = NULL;
            nav->nt = nav->ntmax = 0;
            return NULL;
    ...
    if (!(p->data = (double*)malloc(sizeof(double) * n)) || !(p->rms = (float*)malloc(sizeof(float) * n))) {
        return NULL;
    }
    for (i = 0; i < n; i++) {
        p->data[i] = 0.0;
...
static void readionexdcb(FILE* fp, double* dcb, double* rms) {
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        ...
        if (strstr(label, "PRN / BIAS / RMS") == label) {
            ...
            if (!(sat = satid2no(id))) {
                trace(NULL, 2, "ionex invalid satellite: %s\n", id);
                continue;
            }
...
/* read ionex header ---------------------------------------------------------*/
static double readionexh(FILE* fp, double* lats, double* lons, double* hgts, double* rb, double* nexp, double* dcb,
                         double* rms) {
    double ver = 0.0;
    char buff[1024], *label;

    trace(NULL, 3, "readionexh:\n");
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        ...
        } else if (strstr(label, "START OF AUX DATA") == label && strstr(buff, "DIFFERENTIAL CODE BIASES")) {
...
/* read ionex body -----------------------------------------------------------*/
static int readionexb(FILE* fp, const double* lats, const double* lons, const double* hgts, double rb, double nexp,
                      nav_t* nav) {
    tec_t* p = NULL;
    gtime_t time = {0};
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        ...
        if (strstr(label, "START OF TEC MAP") == label) {
            ...
            if (str2time(buff, 0, 36, &time)) {
                trace(NULL, 2, "ionex epoch invalid: %-36.36s\n", buff);
                continue;
            }
            if (type == 2) {
...
void readtec(const char* file, nav_t* nav, int opt) {
    ...
    for (i = 0; i < n; i++) {
        if (!(fp = fopen(efiles[i], "r"))) {
            trace(NULL, 2, "ionex file open error %s\n", efiles[i]);
            continue;
        }
        /* read ionex header */
        if (readionexh(fp, lats, lons, hgts, &rb, &nexp, dcb, rms) <= 0.0) {
            trace(NULL, 2, "ionex file format error %s\n", efiles[i]);
            continue;
        }
        /* read ionex body */
...
static int interptec(const tec_t* tec, int k, const double* posp, double* value, double* rms) {
    ...
    trace(NULL, 3, "interptec: k=%d posp=%.2f %.2f\n", k, posp[0] * R2D, posp[1] * R2D);
    *value = *rms = 0.0;
...
static int iondelay(gtime_t time, const tec_t* tec, const double* pos, const double* azel, int opt, double* delay,
                    double* var) {
    const double fact = 40.30E16 / FREQ1 / FREQ1; /* tecu->L1 iono (m) */
    double fs, posp[3] = {0}, vtec, rms, hion, rp;
    int i;

    trace(NULL, 3, "iondelay: time=%s pos=%.1f %.1f azel=%.1f %.1f\n", time_str(time, 0), pos[0] * R2D, pos[1] * R2D,
    ...
    *delay = *var = 0.0;
    ...
    trace(NULL, 4, "iondelay: delay=%7.2f std=%6.2f\n", *delay, sqrt(*var));
...
int iontec(gtime_t time, const nav_t* nav, const double* pos, const double* azel, int opt, double* delay, double* var) {
    ...
    trace(NULL, 3, "iontec  : time=%s pos=%.1f %.1f azel=%.1f %.1f\n", time_str(time, 0), pos[0] * R2D, pos[1] * R2D,
          azel[0] * R2D, azel[1] * R2D);
    ...
    if (i == 0 || i >= nav->nt) {
        trace(NULL, 2, "%s: tec grid out of period\n", time_str(time, 0));
        return 0;
    }
    if ((tt = timediff(nav->tec[i].time, nav->tec[i - 1].time)) == 0.0) {
        trace(NULL, 2, "tec grid time interval error\n");
        return 0;
    }
    ...
    if (!stat[0] && !stat[1]) {
        trace(NULL, 2, "%s: tec grid out of area pos=%6.2f %7.2f azel=%6.1f %5.1f\n", time_str(time, 0), pos[0] * R2D,
    ...
    trace(NULL, 3, "iontec  : delay=%5.2f std=%5.2f\n", *delay, sqrt(*var));
    return 1;
}
