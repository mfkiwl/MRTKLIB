...
eph_t* seleph(gtime_t time, int sat, int iode, const nav_t* nav) {
    ...
    for (i = 0; i < nav->n; i++) {
        ...
        if (sys == SYS_CMP) {
            if (iode >= 0 && ((int)nav->eph[i].toes % 2048) != (iode * 8) % 2048) {
                continue;
            }
        } else {
...
static int ephclk(gtime_t time, gtime_t teph, int sat, const nav_t* nav, double* dts) {
    ...
    if (sys == SYS_GPS || sys == SYS_GAL || sys == SYS_QZS || sys == SYS_CMP || sys == SYS_IRN) {
        ...
        *dts = eph2clk(time, eph);
        ...
        if (fabs(geph->taun) > 1.0) {
            return 0; /* reject corrupted GLO clock data */
        }
...
int ephpos(gtime_t time, gtime_t teph, int sat, const nav_t* nav, int iode, double* rs, double* dts, double* var,
           int* svh) {
    eph_t* eph;
    geph_t* geph;
    seph_t* seph;
    ...
    *svh = -1;
    ...
    if (sys == SYS_GPS || sys == SYS_GAL || sys == SYS_QZS || sys == SYS_CMP || sys == SYS_IRN) {
        ...
        *svh = eph->svh;
        ...
        if (fabs(geph->taun) > 1.0) {
            return 0; /* reject corrupted GLO clock data (consistent with ephclk) */
        }
...
static int satpos_sbas(gtime_t time, gtime_t teph, int sat, const nav_t* nav, double* rs, double* dts, double* var,
                       int* svh) {
    const sbssatp_t* sbs;
    int i;
...
extern void set_ssr_ch_idx(int ch) {
...
static int satpos_ssr(gtime_t time, gtime_t teph, int sat, const nav_t* nav, int opt, double* rs, double* dts,
                      double* var, int* svh) {
    const ssr_t* ssr;
    eph_t* eph;
 ...
 *-----------------------------------------------------------------------------*/
extern int satpos(gtime_t time, gtime_t teph, int sat, int ephopt, const nav_t* nav, double* rs, double* dts,
                  double* var, int* svh) {
...
extern void satposs(gtime_t teph, const obsd_t* obs, int n, const nav_t* nav, int ephopt, double* rs, double* dts,
    ...
    for (i = 0; i < n && i < 2 * MAXOBS; i++) {
        trace(NULL, 4, "%s sat=%2d rs=%13.3f %13.3f %13.3f dts=%12.3f var=%7.3f svh=%02X\n", time_str(time[i], 6),
              obs[i].sat, rs[i * 6], rs[1 + i * 6], rs[2 + i * 6], dts[i * 2] * 1E9, var[i], svh[i]);
    }
}
