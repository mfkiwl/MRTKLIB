 ...
 *-----------------------------------------------------------------------------*/
biass_t* getbiass(void) { return &biass; }
    ...
    for (i = 0; i < MAXSTA; i++) {
        for (j = 0; j < MAXBSNXSYS; j++) {
            if (biass.sta[i].syscb[j] != NULL) {
                freebia(&biass.sta[i].syscb[j], &biass.sta[i].nsyscb[j], &biass.sta[i].nsyscbmax[j]);
            }
        }
        for (j = 0; j < MAXSAT; j++) {
            if (biass.sta[i].satcb[j] != NULL) {
                freebia(&biass.sta[i].satcb[j], &biass.sta[i].nsatcb[j], &biass.sta[i].nsatcbmax[j]);
            }
        }
    }
}
            ...
            case 2: /* BIAS/SOLUTION */
                ...
                if (strlen(sta) == 0) { /* satellite */
                    ...
                    if (sig1[0] == 'C') { /* code bias */
                        addbia(&bia, &biass.sat.cb[sat - 1], &biass.sat.ncb[sat - 1], &biass.sat.ncbmax[sat - 1]);
                    } else { /* phase bias */
                        addbia(&bia, &biass.sat.pb[sat - 1], &biass.sat.npb[sat - 1], &biass.sat.npbmax[sat - 1]);
                    }
                } else { /* station */
                    ...
                    if (sat == 0) { /* system code bias */
                        ...
                        addbia(&bia, &biass.sta[i].syscb[j], &biass.sta[i].nsyscb[j], &biass.sta[i].nsyscbmax[j]);
                    } else { /* satellite code bias*/
                        addbia(&bia, &biass.sta[i].satcb[sat - 1], &biass.sta[i].nsatcb[sat - 1],
                               &biass.sta[i].nsatcbmax[sat - 1]);
                    }
                }
                trace(NULL, 4, "readbsnx %8d,%4s,%-9s, %3s, %3s, %3d, %3s, %3d, %s, %8d,%10.4f,%10.4f,%10.4f,%10.4f\n",
                      ret, typeid[bia.type], sta, satid, sig1, bia.code[0], sig2, bia.code[1], time_str(bia.t0, 0),
                      (int)timediff(bia.t1, bia.t0), bia.val, bia.valstd, bia.slp, bia.slpstd);
                ret++;
        }
    }
    fclose(fp);
...
static void outbsnxsold(FILE* fp, int cpflag, int n, const bia_t* biad, char* sat, char* sta) {
    int i;
    ...
    for (i = 0; i < n; i++) {
        ...
        fprintf(fp, "%04.0f:%03.0f:%05.0f %04.0f:%03.0f:%05.0f %-4s %21.14E %11.5E %21.14E %11.5E\n", sy, sdoy, stod,
                ey, edoy, etod, "ns", biad[i].val * M2NS, biad[i].valstd * M2NS, biad[i].slp * M2NS,
                biad[i].slpstd * M2NS);
    }
}
...
void outbsnxsol(FILE* fp) {
    ...
    fprintf(fp,
            ...
            "__ESTIMATED_SLOPE____ _STD_DEV___\n");
    ...
    for (i = 0; i < biass->nsta; i++) {
        for (j = 0; j < MAXBSNXSYS; j++) {
            ...
            outbsnxsold(fp, 0, biass->sta[i].nsyscb[j], biass->sta[i].syscb[j], satid, biass->sta[i].name);
        }
    }
    ...
    for (i = 0; i < biass->nsta; i++) {
        for (j = 0; j < MAXSAT; j++) {
            satno2id(j + 1, satid);
            outbsnxsold(fp, 0, biass->sta[i].nsatcb[j], biass->sta[i].satcb[j], satid, biass->sta[i].name);
        }
    }
    fprintf(fp, "-BIAS/SOLUTION\n");
    fprintf(fp, "%%=ENDBIA\n");
...
int udosb_sat(osb_t* osb, gtime_t gt, int mode) {
    ...
    for (i = 0; i < MAXSAT; i++) {
        for (j = 0; j < biass->sat.ncb[i]; j++) {
            ...
            if (mode || ((timediff(bias->t0, gt) <= 0.0) && (timediff(bias->t1, gt) >= 0.0))) {
