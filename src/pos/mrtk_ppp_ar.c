...
static int exc_sat_sd(rtk_t* rtk, const obsd_t* obs, int exc, double el, int nf, int sys) {
    ...
    trace(NULL, 4, "exc_sat_sd(%4d): sat=%d\n", __LINE__, obs->sat);

    return 1;
}
/* generate satellite SD (single-difference) ---------------------------------*/
static int gen_sat_sd(rtk_t* rtk, const obsd_t* obs, int n, const int* exc, const double* azel, int nf, int* sat1,
                      int* sat2, double* el1, double* el2) {
    ...
    trace(NULL, 3, "gen_sat_sd(%4d): nf=%d arsys=%d elmaskar=%2.0f\n", __LINE__, nf, rtk->opt.arsys,
          rtk->opt.elmaskar * R2D);
    ...
    for (i = 0; sys[i]; i++) { /* for each system */
        ...
        if (maxsat == 0) {
            trace(NULL, 4, "gen_sat_sd(%4d): no reference satellite sys=%d maxsat=%3d maxel=%4.1f\n", __LINE__, sys[i],
                  maxsat, maxel * R2D);
            continue;
        }
        trace(NULL, 4, "gen_sat_sd(%4d): sys=%d maxsat=%3d maxel=%4.1f\n", __LINE__, sys[i], maxsat, maxel * R2D);
        ...
        for (j = 0; j < m; j++) {
            ...
            satno2id(sat[j], s1);
            satno2id(maxsat, s2);
            trace(NULL, 3, "gen_sat_sd(%4d): %s %s el=%4.1f %4.1f[deg]\n", __LINE__, s1, s2, el[j] * R2D, maxel * R2D);
        }
    }
    trace(NULL, 4, "gen_sat_sd(%4d): ns=%d\n", __LINE__, ns);
    return ns; /* # of Satellite */
}
/* write debug trace for PAR -------------------------------------------------*/
static void write_trace_amb(rtk_t* rtk, const double* a, const double* Q, int na, const int* sat1s, const int* sat2s,
                            const int* frqs, const char label[NFREQ][10], int step) {
    char s1[32], s2[32];
    double ep[6] = {0.0};
    gtime_t t0;
    int i;
    trace(NULL, 2, "write_trace_amb: EPOCH=%s\n", time_str(rtk->sol.time, 0));
    ...
    for (i = 0; i < na; i++) {
        satno2id(sat1s[i], s1);
        satno2id(sat2s[i], s2);
        trace(NULL, 2, "write_trace_amb:, STEP_%d,  %5.0f, %s_%s, %s, %14.3f, %7.3f, %7.3f\n", step,
              timediff(rtk->sol.time, t0), s2, s1, label[frqs[i]], a[i], sqrt(Q[i + i * na]), ROUND(a[i]) - a[i]);
    }
}
static void write_trace_sd_amb(rtk_t* rtk, const double* x, const double* P, const int* sat1, const int* sat2, int ns,
                               int nf, int* sat1s, int* sat2s, int* frqs, double* a, int post, int step) {
    ...
    trace(NULL, 4, "write_trace_sd_amb(%4d): ns=%d nf=%d\n", __LINE__, ns, nf);
...
static int search_amb_ewl(rtk_t* rtk, const double* x, const double* P, const int* sat1, const int* sat2, int ns,
                          int nf, int* sat1s, int* sat2s, int* frqs, double* a, double* N, double* D) {
    ...
    for (i = 0; i < ns; i++) {
        ...
        for (f = 2; f < rtk->opt.nf; f++) {
            ...
            if (lam[f1] == 0.0 || lam[f2] == 0.0 || x[j1] == 0.0 || x[j2] == 0.0 || x[k1] == 0.0 || x[k2] == 0.0) {
                trace(NULL, 3, "search_amb_ewl(%4d): skipped %s %s x(j1,j2,k1,k2)=%8.3f %8.3f %8.3f %8.3f\n", __LINE__,
            ...
            n = ROUND(b);
            frac = fabs(n - b);
            var = P[j1 + j1 * rtk->nx] / SQR(lam[f1]) + P[j2 + j2 * rtk->nx] / SQR(lam[f2]) -
                  2.0 * P[j1 + j2 * rtk->nx] / (lam[f1] * lam[f2]);
            var += P[k1 + k1 * rtk->nx] / SQR(lam[f1]) + P[k2 + k2 * rtk->nx] / SQR(lam[f2]) -
                   2.0 * P[k1 + k2 * rtk->nx] / (lam[f1] * lam[f2]);

            if (sqrt(var) > th_std || frac > th_frac) {
                trace(NULL, 2,
                      "search_amb_ewl(%4d): excluded %s %s N,a= %8.3f %8.3f "
                      "sig=%8.3f frac=%8.3f/%8.3f std=%8.3f/%8.3f\n",
                      __LINE__, s1, s2, n, b, sqrt(var), frac, th_frac, sqrt(var), th_std);
                continue;
            }
            ...
            if (strstr(rtk->opt.pppopt, "-TRACE_AR")) {
                trace(NULL, 3, "search_amb_ewl(%4d): %s %s N,a= %8.3f %8.3f sig=%8.3f\n", __LINE__, s1, s2, n, b,
                      sqrt(var));
            }
        }
    }
    trace(NULL, 4, "search_amb_ewl(%4d): number of fixed extra wide-lane ambiguity = %d / %d\n", __LINE__, na, ns);
    return na;
}
/* search wide-lane integer ambiguity ----------------------------------------*/
static int search_amb_wl(rtk_t* rtk, const double* x, const double* P, int* sat1, int* sat2, int ns, int nf, int* sat1s,
                         int* sat2s, int* frqs, double* a, double* N, double* D) {
    int i, j1, j2, k1, k2, na = 0;
    double lam[NFREQ];
    double b, n, var, frac;
    double th_frac = MAX_FRAC_WL_FIX, th_std = MAX_STD_WL_FIX;
    ...
    for (i = 0; i < ns; i++) {
        ...
        if (lam[0] == 0.0 || lam[1] == 0.0 || x[j1] == 0.0 || x[j2] == 0.0 || x[k1] == 0.0 || x[k2] == 0.0) {
            trace(NULL, 3, "search_amb_wl(%4d): excluded %s %s x(j1,j2,k1,k2)=%.3f %.3f %.3f %.3f\n", __LINE__, s1, s2,
                  x[j1], x[j2], x[k1], x[k2]);
        ...
        n = ROUND(b);
        frac = fabs(n - b);
        var = P[j1 + j1 * rtk->nx] / SQR(lam[0]) + P[j2 + j2 * rtk->nx] / SQR(lam[1]) -
              2.0 * P[j1 + j2 * rtk->nx] / (lam[0] * lam[1]);
        var += P[k1 + k1 * rtk->nx] / SQR(lam[0]) + P[k2 + k2 * rtk->nx] / SQR(lam[1]) -
               2.0 * P[k1 + k2 * rtk->nx] / (lam[0] * lam[1]);

        if (sqrt(var) > th_std || frac > th_frac) {
            trace(NULL, 2,
                  "search_amb_wl(%4d): excluded %s %s N,a= %8.3f %8.3f "
                  "sig=%8.3f frac=%8.3f/%8.3f std=%8.3f/%8.3f\n",
                  __LINE__, s1, s2, n, b, sqrt(var), frac, th_frac, sqrt(var), th_std);
        ...
        if (strstr(rtk->opt.pppopt, "-TRACE_AR")) {
            trace(NULL, 3, "search_amb_wl(%4d): %s %s N,a= %8.3f %8.3f sig=%8.3f\n", __LINE__, s1, s2, n, b, sqrt(var));
        }
    }
    trace(NULL, 4, "search_amb_wl(%4d): number of fixed wide-lane ambiguity = %d / %d\n", __LINE__, na, ns);
    return na;
}
/* generate SD-matrix of narrow-lane ambiguity --------------------------------*/
static int gen_sd_matrix_n1(rtk_t* rtk, const double* x, const double* P, const int* sat1, const int* sat2, int ns,
                            int nf, int* sat1s, int* sat2s, int* frqs, double* a, double* Q, double* D) {
    ...
    trace(NULL, 3, "gen_sd_matrix_n1(%4d): ns=%d proc_time=%d [ms]\n", __LINE__, ns, (int)(tickget() - tick));
    ...
    trace(NULL, 3, "gen_sd_matrix_n1(%4d): na=%d proc_time=%d [ms]\n", __LINE__, na, (int)(tickget() - tick));
    return na;
}
/* search integer ambiguity by LAMBDA ----------------------------------------*/
static int search_amb_lambda(rtk_t* rtk, const obsd_t* obs, int n, const double* azel, const int* sat1s,
                             const int* sat2s, const int* frq, const double* x, const double* P, const double* D,
                             const double* Q, const double* a, double* N, int na, int* exc_sat_flag) {
    ...
    if (lambda(na, 2, a, Q, N, s) || s[0] <= 0.0) {
        trace(NULL, 2, "search_amb_lambda(%4d): error lambda\n", __LINE__);
        return 0;
    }
    ...
    if (na - MIN_AMB_RES < (sizeof(thres_fact) / sizeof(thres_fact[0]))) {
        ...
        trace(NULL, 2, "search_amb_lambda(%4d): inflated threshold na-MIN_AMB_RES=%d thres_fact=%.2f thres=%.2f\n",
              __LINE__, na - MIN_AMB_RES, thres_fact[na - MIN_AMB_RES], thres);
    }
    for (i = 0; i < MAXSAT; i++) {
        exc_sat_flag[i] = 0;
    }
    ...
    if (trace_AR) {
        ...
        trace(NULL, 2, "search_amb_lambda(%4d): first candidate\n", __LINE__);
        for (i = 0; i < na; i++) {
            satno2id(sat1s[i], s1);
            satno2id(sat2s[i], s2);
            trace(NULL, 2, "search_amb_lambda(%4d): i= %2d na= %2d %s %s N,a= %8.3f %8.3f frac=%8.3f\n", __LINE__, i,
                  na, s1, s2, N[i], a[i], N[i] - a[i]);
        }
        trace(NULL, 2, "search_amb_lambda(%4d): second candidate\n", __LINE__);
        for (i = 0; i < na; i++) {
            satno2id(sat1s[i], s1);
            satno2id(sat2s[i], s2);
            trace(NULL, 2, "search_amb_lambda(%4d): i= %2d na= %2d %s %s N,a= %8.3f %8.3f frac=%8.3f\n", __LINE__, i,
                  na, s1, s2, N[i + na], a[i], N[i + na] - a[i]);
        }
        trace(NULL, 2, "search_amb_lambda(%4d): difference\n", __LINE__);
        for (i = 0; i < na; i++) {
            satno2id(sat1s[i], s1);
            satno2id(sat2s[i], s2);
            trace(NULL, 2, "search_amb_lambda(%4d): i= %2d na= %2d %s %s N1,N2= %8.3f %8.3f diff=%8.3f\n", __LINE__, i,
                  na, s1, s2, N[i], N[i + na], N[i] - N[i + na]);
        }
    }

    rtk->sol.ratio = (float)MIN(s[1] / s[0], 999.9);
    if (s[1] / s[0] < thres) {
        trace(NULL, 2, "search_amb_lambda(%4d): failed to fix. na=%d ratio=%.2f/%.2f proc_time=%d [ms]\n", __LINE__, na,
              s[1] / s[0], thres, (int)(tickget() - tick));
        return 0;
    }
    ...
    trace(NULL, 2, "search_amb_lambda(%4d): successed to fix. na=%d ratio=%.2f/%.2f proc_time=%d [ms]\n", __LINE__, na,
          s[1] / s[0], thres, (int)(tickget() - tick));
    return na;
}
/* update states with integer ambiguity constraints --------------------------*/
static int update_states(double* x, double* P, const double* D, int nx, const double* N, const double* a, int na,
    ...
    for (i = 0; i < na; i++) {
        ...
        R[i + i * na] = var;
        trace(NULL, 4,
              "update_states(%4d): sat=%3d frqs=%d fn=%d i=%2d na=%2d N,a=%10.3f %10.3f frac=%8.3f sig=%6.3f\n",
              __LINE__, sats[i], frqs[i], freq_idx2freq_num(sys, frqs[i]), i, na, N[i], a[i], b[i], sqrt(var));
    }
    if ((info = filter(x, P, D, b, R, nx, na))) {
        trace(NULL, 1, "update_states(%4d): filter error (info=%d)\n", __LINE__, info);
    }
    free(R);

    trace(NULL, 3, "update_states(%4d): na=%d proc_time=%d [ms]\n", __LINE__, na, (int)(tickget() - tick));
    return info;
}
/* exclude satellite for partial AR ------------------------------------------*/
static int exc_sat_par(rtk_t* rtk, int* sat1, int* sat2, const double* el1, const double* el2, int ns,
                       const int* exc_sat_flag) {
    int i, minidx = 0, sys;
    double minel = 9999.9;
    ...
    for (i = 0; i < ns; i++) {
        ...
        trace(NULL, 4, "exc_sat_par(%4d): i=%2d sat1=%3d sat2=%3d el=%4.1f %4.1f minidx=%2d minel=%.3f\n", __LINE__, i,
              sat1[i], sat2[i], el1[i] * R2D, el2[i] * R2D, minidx, minel * R2D);
    }
    satno2id(sat1[minidx], s1);
    satno2id(sat2[minidx], s2);
    trace(NULL, 2, "exc_sat_par(%4d): excluded %s %s sat1=%3d sat2=%3d el=%4.1f %4.1f\n", __LINE__, s1, s2,
          sat1[minidx], sat2[minidx], el1[minidx] * R2D, el2[minidx] * R2D);
    sat1[minidx] = 0;
    sat2[minidx] = 0;

    for (i = 0; i < ns; i++) {
        trace(NULL, 4, "exc_sat_par(%4d): i=%2d sat1=%3d sat2=%3d el1=%4.1f\n", __LINE__, i, sat1[i], sat2[i],
              el1[i] * R2D);
    }
...
static int ppp_amb_ILS(rtk_t* rtk, const obsd_t* obs, int n, int* exc, const nav_t* nav, const double* azel, double* x,
                       double* P) {
    ...
    time2str(rtk->sol.time, tstr, 0);
    trace(NULL, 3, "ppp_amb_ILS(%4d): nf=%d\n", __LINE__, nf);

    /* generate satellite SD */
    ns = gen_sat_sd(rtk, obs, n, exc, azel, nf, sat1, sat2, el1, el2);
    if (ns <= 0) {
        trace(NULL, 2, "ppp_amb_ILS(%4d): no generated satellite SD ns= %d\n", __LINE__, ns);
        return 0;
    }
    trace(NULL, 3, "ppp_amb_ILS(%4d): %s generate satellite SD ns= %d\n", __LINE__, tstr, ns);
    ...
    if (info) {
        trace(NULL, 1, "ppp_amb_ILS(%4d): %s update error\n", __LINE__, tstr);
        free(D);
        free(Q);
        free(xp);
        free(Pp);
    ...
    trace(NULL, 3, "ppp_amb_ILS(%4d): %s extra wide-lane ambiguity fixed. nfix=%2d nsat=%2d\n", __LINE__, tstr, na, ns);
    ...
    if (na == 0) {
        trace(NULL, 2, "ppp_amb_ILS(%4d): %s wide-lane ambiguity search error\n", __LINE__, tstr);
        rtk->sol.age = 0;
        free(D);
        free(Q);
        free(xp);
    ...
    trace(NULL, 3, "ppp_amb_ILS(%4d): %s wide-lane ambiguity fixed. nfix=%2d nsat=%2d proc_time=%d [ms]\n", __LINE__,
    ...
    if (norm(std, 3) < rtk->opt.thresar[1]) {
        ...
        na = 0;
        i = 0;
        trace(NULL, 2, "ppp_amb_ILS(%4d): %s std is greater than the threshold. std=%.2f / %.2f\n", __LINE__, tstr,
              norm(std, 3), rtk->opt.thresar[1]);
    }
    if (na == 0) {
        rtk->sol.age = (float)i;
        free(D);
        ...
        free(xp);
        free(Pp);
        trace(NULL, 2, "ppp_amb_ILS(%4d): %s narrow-lane search error. iter=%d proc_time=%d [ms]\n", __LINE__, tstr,
              (int)rtk->sol.age, (int)(tickget() - tick));
    ...
    if (norm(std, 3) > MAX_STD_FIX) {
        ...
        free(xp);
        free(Pp);
        trace(NULL, 2, "ppp_amb_ILS(%4d): %s failed to std test. std=%.3f / %.3f iter=%d proc_time=%d [ms]\n", __LINE__,
    ...
    trace(NULL, 3, "ppp_amb_ILS(%4d): %s ambiguity fixed. nfix=%2d nsat=%2d std=%.3f iter=%d proc_time=%d [ms]\n",
          __LINE__, tstr, na, ns, norm(std, 3), (int)rtk->sol.age, (int)(tickget() - tick));
...
extern int ppp_ar(mrtk_ctx_t* ctx, rtk_t* rtk, const obsd_t* obs, int n, int* exc, const nav_t* nav, const double* azel,
                  double* x, double* P) {
