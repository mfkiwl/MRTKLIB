...
static int decode_nmearmc(char** val, int n, sol_t* sol) {
    ...
    trace(NULL, 4, "decode_nmearmc: n=%d\n", n);
    ...
    if ((act != 'A' && act != 'V') || (ns != 'N' && ns != 'S') || (ew != 'E' && ew != 'W')) {
        trace(NULL, 3, "invalid nmea rmc format\n");
        return 0;
    }
    ...
    trace(NULL, 5, "decode_nmearmc: %s rr=%.3f %.3f %.3f stat=%d ns=%d vel=%.2f dir=%.0f ang=%.0f mew=%c mode=%c\n",
...
static int decode_nmeazda(char** val, int n, sol_t* sol) {
    ...
    trace(NULL, 4, "decode_nmeazda: n=%d\n", n);
...
static int decode_nmeagga(char** val, int n, sol_t* sol) {
    ...
    trace(NULL, 4, "decode_nmeagga: n=%d\n", n);
    ...
    if ((ns != 'N' && ns != 'S') || (ew != 'E' && ew != 'W')) {
        trace(NULL, 3, "invalid nmea gga format\n");
        return 0;
    }
    if (sol->time.time == 0) {
        trace(NULL, 3, "no date info for nmea gga\n");
        return 0;
    }
    ...
    trace(NULL, 5, "decode_nmeagga: %s rr=%.3f %.3f %.3f stat=%d ns=%d hdop=%.1f ua=%c um=%c\n", time_str(sol->time, 0),
...
static int test_nmea(const char* buff) {
    if (strlen(buff) < 6 || buff[0] != '$') {
        return 0;
    }
    return !strncmp(buff + 1, "GP", 2) || !strncmp(buff + 1, "GA", 2) || /* NMEA 4.10 [1] */
           !strncmp(buff + 1, "GL", 2) || !strncmp(buff + 1, "GN", 2) || !strncmp(buff + 1, "GB", 2) ||
           !strncmp(buff + 1, "GQ", 2) || /* NMEA 4.11 [2] */
           !strncmp(buff + 1, "GI", 2) || !strncmp(buff + 1, "BD", 2) || !strncmp(buff + 1, "QZ", 2); /* extension */
}
/* test solution status message header ---------------------------------------*/
static int test_solstat(const char* buff) {
    if (strlen(buff) < 7 || buff[0] != '$') {
        return 0;
    }
    return !strncmp(buff + 1, "POS", 3) || !strncmp(buff + 1, "VELACC", 6) || !strncmp(buff + 1, "CLK", 3) ||
           !strncmp(buff + 1, "ION", 3) || !strncmp(buff + 1, "TROP", 4) || !strncmp(buff + 1, "HWBIAS", 6) ||
           !strncmp(buff + 1, "TRPG", 4) || !strncmp(buff + 1, "AMB", 3) || !strncmp(buff + 1, "SAT", 3);
}
...
static int decode_nmea(char* buff, sol_t* sol) {
    ...
    trace(NULL, 4, "decode_nmea: buff=%s\n", buff);
...
static int decode_solxyz(char* buff, const solopt_t* opt, sol_t* sol) {
    ...
    if (i < n) {
        sol->stat = (uint8_t)val[i++];
    }
    if (i < n) {
        sol->ns = (uint8_t)val[i++];
    }
    if (i + 3 <= n) {
        P[0] = SQR(val[i]);
        i++; /* sdx */
...
static int decode_solsss(char* buff, sol_t* sol) {
    ...
    trace(NULL, 4, "decode_solsss:\n");

    if (sscanf(buff, "$POS,%d,%lf,%d,%lf,%lf,%lf,%lf,%lf,%lf", &week, &tow, &solq, pos, pos + 1, pos + 2, std, std + 1,
               std + 2) < 6) {
        return 0;
    }
...
static int decode_solpos(char* buff, const solopt_t* opt, sol_t* sol) {
    ...
    trace(NULL, 4, "decode_solpos: buff=%s\n", buff);
...
static void decode_refpos(char* buff, const solopt_t* opt, double* rb) {
    ...
    trace(NULL, 3, "decode_refpos: buff=%s\n", buff);
...
static int decode_sol(char* buff, const solopt_t* opt, sol_t* sol, double* rb) {
    ...
    trace(NULL, 4, "decode_sol: buff=%s\n", buff);
...
static void decode_solopt(char* buff, solopt_t* opt) {
    ...
    trace(NULL, 4, "decode_solhead: buff=%s\n", buff);

    if (strncmp(buff, COMMENTH, 1) && strncmp(buff, "+", 1)) {
        return;
    }
...
int inputsol(uint8_t data, gtime_t ts, gtime_t te, double tint, int qflag, const solopt_t* opt, solbuf_t* solbuf) {
    ...
    trace(NULL, 4, "inputsol: data=0x%02x\n", data);
    ...
    if (!strncmp((char*)solbuf->buff, MSG_DISCONN, strlen(MSG_DISCONN) - 2)) {
        trace(NULL, 3, "disconnect received\n");
        return -1;
    }
    /* decode solution */
    sol.time = solbuf->time;
...
/* read solution data --------------------------------------------------------*/
static int readsoldata(FILE* fp, gtime_t ts, gtime_t te, double tint, int qflag, const solopt_t* opt,
                       solbuf_t* solbuf) {
    int c;

    trace(NULL, 3, "readsoldata:\n");
...
static int sort_solbuf(solbuf_t* solbuf) {
    ...
    if (!(solbuf_data = (sol_t*)realloc(solbuf->data, sizeof(sol_t) * solbuf->n))) {
        trace(NULL, 1, "sort_solbuf: memory allocation error\n");
        free(solbuf->data);
        solbuf->data = NULL;
...
int readsolt(char* files[], int nfile, gtime_t ts, gtime_t te, double tint, int qflag, solbuf_t* solbuf) {
    ...
    trace(NULL, 3, "readsolt: nfile=%d\n", nfile);

    initsolbuf(solbuf, 0, 0);

    for (i = 0; i < nfile; i++) {
        if (!(fp = fopen(files[i], "rb"))) {
            trace(NULL, 2, "readsolt: file open error %s\n", files[i]);
            continue;
        }
        ...
        if (!readsoldata(fp, ts, te, tint, qflag, &opt, solbuf)) {
            trace(NULL, 2, "readsolt: no solution in %s\n", files[i]);
        }
        fclose(fp);
    }
...
int addsol(solbuf_t* solbuf, const sol_t* sol) {
    ...
    if (solbuf->n >= solbuf->nmax) {
        ...
        if (!(solbuf_data = (sol_t*)realloc(solbuf->data, sizeof(sol_t) * solbuf->nmax))) {
            trace(NULL, 1, "addsol: memory allocation error\n");
            free(solbuf->data);
            solbuf->data = NULL;
            solbuf->n = solbuf->nmax = 0;
            return 0;
...
void initsolbuf(solbuf_t* solbuf, int cyclic, int nmax) {
    ...
    trace(NULL, 3, "initsolbuf: cyclic=%d nmax=%d\n", cyclic, nmax);
    ...
    if (cyclic) {
        ...
        if (!(solbuf->data = malloc(sizeof(sol_t) * nmax))) {
            trace(NULL, 1, "initsolbuf: memory allocation error\n");
            return;
        }
        solbuf->nmax = nmax;
    }
...
void freesolstatbuf(solstatbuf_t* solstatbuf) {
    trace(NULL, 3, "freesolstatbuf: n=%d\n", solstatbuf->n);
...
static int sort_solstat(solstatbuf_t* statbuf) {
    ...
    if (!(statbuf_data = realloc(statbuf->data, sizeof(solstat_t) * statbuf->n))) {
        trace(NULL, 1, "sort_solstat: memory allocation error\n");
        free(statbuf->data);
        statbuf->data = NULL;
...
static int decode_solstat(char* buff, solstat_t* stat) {
    ...
    trace(NULL, 4, "decode_solstat: buff=%s\n", buff);
    ...
    n = sscanf(buff, "$SAT%d%lf%s%d%lf%lf%lf%lf%d%lf%d%d%d%d%d%d", &week, &tow, id, &frq, &az, &el, &resp, &resc, &vsat,
               &snr, &fix, &slip, &lock, &outc, &slipc, &rejc);

    if (n < 15) {
        trace(NULL, 2, "invalid format of solution status: %s\n", buff);
        return 0;
    }
    if ((sat = satid2no(id)) <= 0) {
        trace(NULL, 2, "invalid satellite in solution status: %s\n", id);
        return 0;
    }
    *stat = stat0;
...
static void addsolstat(solstatbuf_t* statbuf, const solstat_t* stat) {
    ...
    trace(NULL, 4, "addsolstat:\n");

    if (statbuf->n >= statbuf->nmax) {
        statbuf->nmax = statbuf->nmax == 0 ? 8192 : statbuf->nmax * 2;
        if (!(statbuf_data = (solstat_t*)realloc(statbuf->data, sizeof(solstat_t) * statbuf->nmax))) {
            trace(NULL, 1, "addsolstat: memory allocation error\n");
            free(statbuf->data);
            statbuf->data = NULL;
            statbuf->n = statbuf->nmax = 0;
            return;
...
static int readsolstatdata(FILE* fp, gtime_t ts, gtime_t te, double tint, solstatbuf_t* statbuf) {
    solstat_t stat = {{0}};
    char buff[MAXSOLMSG + 1];

    trace(NULL, 3, "readsolstatdata:\n");
...
int readsolstatt(char* files[], int nfile, gtime_t ts, gtime_t te, double tint, solstatbuf_t* statbuf) {
    ...
    trace(NULL, 3, "readsolstatt: nfile=%d\n", nfile);
    ...
    for (i = 0; i < nfile; i++) {
        ...
        if (!(fp = fopen(path, "r"))) {
            trace(NULL, 2, "readsolstatt: file open error %s\n", path);
            continue;
        }
        /* read solution status data */
        if (!readsolstatdata(fp, ts, te, tint, statbuf)) {
            trace(NULL, 2, "readsolstatt: no solution in %s\n", path);
        }
        fclose(fp);
    }
#if 0
...
int readsolstat(char* files[], int nfile, solstatbuf_t* statbuf) {
    ...
    trace(NULL, 3, "readsolstat: nfile=%d\n", nfile);
...
int outnmea_gsa(uint8_t* buff, const sol_t* sol, const ssat_t* ssat) {
    ...
    for (i = 0; nmea_sys[i]; i++) {
        ...
        if (nsat > 0) {
            ...
            p += sprintf(p, ",%3.1f,%3.1f,%3.1f,%d", dop[1], dop[2], dop[3], nmea_sid[i]);
            for (q = s + 1, sum = 0; *q; q++) {
                sum ^= *q; /* check-sum */
            }
            p += sprintf(p, "*%02X\r\n", sum);
...
int outprcopts(uint8_t* buff, const prcopt_t* opt) {
    ...
    const char* s4[] = {"OFF", "Broadcast", "SBAS", "Iono-Free LC", "Estimate TEC", "IONEX TEC", "QZSS Broadcast", "",
                        "",    "",          ""};
