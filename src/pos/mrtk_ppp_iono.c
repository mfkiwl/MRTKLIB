...
static double miono_delay(const nav_t* nav, const int sat, const miono_area_t* a, const double* ll) {
...
static miono_area_t* miono_sel_area(pppiono_t* pppiono, const double* rr, const double* ll) {
    ...
    for (i = 0; i < MIONO_MAX_RID; i++) {
        ...
        for (j = 0; j < MIONO_MAX_ANUM; j++) {
            ...
            trace(NULL, 5, "miono_sel_area: area i=%d,j=%d,sid=%d,ref=%6.2f,%7.2f,dist=%10.3f,min_dist=%10.3f\n", i, j,
                  re->area[j].sid, re->area[j].ref[0], re->area[j].ref[1], dist, min_dist);
            if (dist > min_dist) {
                continue;
            }
...
extern int input_statcorr(pppiono_corr_t* corr, FILE* fp) {
    ...
    */
    ...
    while (fgets(buff, sizeof(buff), fp)) {
        ...
        /* $POS record */
        if (sscanf(buff, "$POS%d%lf%d%lf%lf%lf%lf%lf%lf", &week, &tow, &stat, pos, pos + 1, pos + 2, std, std + 1,
                   std + 2) >= 9) {
        ...
        /* $ION record */
        if (sscanf(buff, "$ION%d%lf%d%s%lf%lf%lf%lf", &week, &tow, &stat, satid, azel, azel + 1, &dly, &dlystd) >= 8) {
            time = gpst2time(week, tow);
...
extern int const_iono_corr(rtk_t* rtk, const obsd_t* obs, const nav_t* nav, const int n, const double* azel,
    ...
    for (i = 0; i < n; i++) {
        ...
        trace(NULL, 2,
              "const_iono_corr: %s,%s,tt=%11d,az=%5.1f,el=%4.1f,sdly=%8.4f,std=%8.4f,v=%8.4f,x=%8.4f,bsys=%8.4f\n",
              satid, tstr, (int)tt, (azel + i * 2)[0] * R2D, (azel + i * 2)[1] * R2D, corr.dly[sat - 1],
              corr.std[sat - 1], v[nv], x[j], bsys[s]);
